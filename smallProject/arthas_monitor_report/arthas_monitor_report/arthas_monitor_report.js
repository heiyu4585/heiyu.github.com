...
/******/ 	__webpack_require__.p = "/arthas_monitor_report";
/******/
...
;(function (global) {
    var ArthasMonitor = function ArthasMonitor() {};
    ...
    ArthasMonitor.prototype = {
        init: function init(options) {
            ...
            this.store.baseData = this.util.deepExtend(baseData, {
        ...
        util: function () {
            function postData(data, url, fn) {
                if (!url) console.log("error------url未设置");
                console.log("当前发送的数据------");
                console.log(data);
                console.log("当前发送的数据------");
            ...
            var debugLog = window.debugLog || function debugLog() {
                ...
                if (typeof debugMode != "undefined" && debugMode && typeof debugMode != null && console.group != undefined) {
                    ...
                    for (var i = 0; i < arguments.length; i++) {
                        if ((0, _typeof3.default)(arguments[i]) == "object") {
        ...
        config: {
            reportVersion: '1.0.0',
            reportPath: 'http://10.0.1.128:7001/ams/catch',
            limitTime: 3000 },
        ...
        getData: function getData() {
            ...
            var baseData = {
                ...
                projectType: /Android|webOS|iPhone|iPod|BlackBerry/i.test(navigator.userAgent) ? "Mobile" : "PC",
                flashVer: getFlashPlayerVersion(),
            ...
            function getAppVersion() {
                ...
                if (!!appVersion) return appVersion;else return null;
            }
            ...
            function getFlashPlayerVersion() {
                ...
                if (hasFlash == 1) return flashVersion;else return null;
            }
        ...
        checkImg: function checkImg() {
            ...
            setInterval(function () {
                console.log("再次执行检查图片");
                console.log("this.store应该有值", _this.store);
                var imgList = checkImgSize();
                if (imgList && imgList.length > 0) {
                    console.log("将要发送的异常图片---");
                    console.log(imgList);
                    console.log("将要发送的异常图片---");
                    this.checkBeforeSend(_this.util.deepExtend(store.baseData, {
                        ext: {
                            bigIMgList: imgList
                        }
                    }), store);
            ...
            function checkImgSize() {
                console.log("当前store---checkImgSize");
                ...
                if (performance.getEntriesByType("resource").length > 0 && _this.store.resourceListLength === performance.getEntriesByType("resource").length) return false;

                var result = [];
                _this.store.resourceListLength = performance.getEntriesByType("resource").length;
        ...
        checkBeforeSend: function checkBeforeSend(data) {
            ...
            if (!!data) {
                if (store.logList && store.logList.length) {
                    for (var i = 0; i < store.logList.length; i++) {
                        if (data.msg && store.logList[i].msg && data.msg === store.logList[i].msg) {
                            if (Math.abs(data.timestamp - store.logList[i].timestamp) >= store.config.limitTime) {
                                ...
                                this.sendData(data, store.config.reportPath);
                                break;
                            }
                        }
                    }
                    console.log("当前store---checkBeforeSend-if");
                    console.log(store);
                    console.log("当前store---checkBeforeSend-if");
                } else {
                    store.logList = [];
                    ...
                    this.sendData(data, store.config.reportPath);
                    console.log("当前store---checkBeforeSend-else");
                    console.log(store);
                    console.log("当前store--checkBeforeSend-else");
                }
            }
        ...
        initWindowErrorHandle: function initWindowErrorHandle(global, store) {
            if (!global) return;
            var _this = this;
            global.onerror = function (msg, url, line, col, error) {
                console.log("有onerror触发---");
                console.log(msg);
                console.log("有onerror触发---");
        ...
        initVueErrorHandle: function initVueErrorHandle(err, metaData) {
            this.checkBeforeSend(this.util.deepExtend(this.store.baseData, {
                metaData: metaData
            }, {
                msg: subStack(err.stack, 10) }));
